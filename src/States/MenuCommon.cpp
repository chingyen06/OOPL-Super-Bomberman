...
Util::Color MenuCommon::DarkText()   { return Util::Color::FromName(Util::Colors::BLACK); }
Util::Color MenuCommon::WhiteText()  { return Util::Color::FromName(Util::Colors::WHITE); }
Util::Color MenuCommon::YellowText() { return Util::Color::FromName(Util::Colors::YELLOW); }
Util::Color MenuCommon::GoldText()   { return Util::Color(230, 150, 20); }

// 按鍵 → 顯示字串。用 SDL scancode 範圍處理字母/數字，特殊鍵用 switch。
std::string MenuCommon::KeyName(Util::Keycode k) {
...
Util::Keycode MenuCommon::PollAnyKey() {
    using K = Util::Keycode;
...
void KeyHint::Remove(App& app) {
    m_Group.Clear(app.Root());
}

// keycap + 動作字一段的版面資料 (建構時算好字寬與膠囊寬)。
MenuCommon::Item::Item(const std::string& keyStr, const std::string& actStr,
...
KeyHint MenuCommon::AddKeyHint(App& app, const std::vector<std::pair<std::string, std::string>>& segs) {
    auto& root = app.Root();
    KeyHint hint;
...
CoinHud MenuCommon::AddCoinHud(App& app) {
    auto& root = app.Root();
